...
extern void change_fan_direction(void);
...
//    #error "请将MCU串口发送函数填入该函数,并删除该行"
    HAL_UART_Transmit(&huart1, (uint8_t*)&value, 1, 0xFFFF);
/*
    //Example:
...
//    #error "请在此处理可下发可上报数据及只上报数据示例,处理完成后删除该行"
    ...
    mcu_dp_bool_update(DPID_SWITCH, gs_fan_status.OnOff); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_MODE, gs_fan_status.e_fan_mode); //枚举型数据上报;
    mcu_dp_value_update(DPID_FAN_SPEED, gs_fan_status.speed); //VALUE型数据上报;
    mcu_dp_enum_update(DPID_FAN_DIRECTION, gs_fan_status.e_fan_direction); //枚举型数据上报;
...
static unsigned char dp_download_mode_handle(const unsigned char value[], unsigned short length)
{
    ...
    switch(mode) {
        ...
        case 1:
					gs_fan_status.e_fan_mode = sleep;
        break;
